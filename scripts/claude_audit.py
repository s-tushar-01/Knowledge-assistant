...
check("text-embedding-3-small" in cfg,  "config.py", "model=text-embedding-3-small", "Correct OpenAI embed model")
...
# Extract the route and the SSE generator body. HTTPException before the generator is OK.
chat_route_body = chat.split("async def chat(request")[1].split("async def chat_sync")[0]
chat_gen_body = chat_route_body.split("async def generate():")[1].split("return StreamingResponse")[0]
check("StreamingResponse" in chat_route_body,    "chat.py", "StreamingResponse",       "StreamingResponse wraps the generator")
check("text/event-stream" in chat_route_body,    "chat.py", "SSE media type",          "media_type=text/event-stream")
...
check("'type': 'error'" in chat_gen_body or '"type": "error"' in chat_gen_body,
      "chat.py", "typed SSE error events",  "Errors emitted as typed SSE {type:error} events")
...
check("SYSTEM_PROMPT.format(" not in prompts, "prompts.py", "no str.format", "No str.format() that would fail on { in chunks")
...
for (passed, component, rule, detail) in findings:
    ...
    if passed:
        ...
        failed_all += 1
    print(f"  {icon:<4s}  [{component:<22s}] {rule}")
    if not passed:
...
if failed_all == 0:
    print("  PASS  All architecture checks passed; system is CLAUDE.md compliant")
else:
    print("  FAIL  Violations found; see items marked FAIL above")
print("=" * 72)
